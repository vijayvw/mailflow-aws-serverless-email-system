...
Follow these steps to deploy MailFlow from scratch.

## 1️⃣ Clone the Repository

```bash
git clone https://github.com/YOUR_USERNAME/mailflow-aws-serverless-email-system.git
cd mailflow-aws-serverless-email-system
```

## 2️⃣ Configure AWS CLI

Make sure your AWS credentials are configured and have permission to create the required AWS resources.
...
Edit `terraform.tfvars` and provide your own values:
...
The `sender_email` must be verified in Amazon SES. During SES Sandbox testing, the recipient email may also need to be verified.

> ⚠️ Never commit your real `terraform.tfvars` file. Keep environment-specific configuration out of Git.
...
## 6️⃣ Review the Infrastructure

```bash
terraform plan
```

Review the resources Terraform will create before applying them.
...
Terraform provisions the S3 buckets, Lambda functions, API Gateway, SES identity, SNS topic, IAM permissions, S3 event trigger, and frontend configuration.

## 8️⃣ Get the Application URL
...
Use the `frontend_url` output to open the MailFlow web application.

## 9️⃣ Create a Recipient CSV
...
1. Open the MailFlow frontend.
2. Select or drag-and-drop the CSV file.
3. Click **Upload & Trigger Email Pipeline**.
4. MailFlow requests a temporary presigned S3 URL.
5. The browser uploads the CSV directly to S3.
6. S3 triggers the Email Processor Lambda.
7. Lambda loads `lambda/email_template.html`.
8. `{{name}}` is replaced with the recipient's name.
9. Amazon SES sends the email.
10. Amazon SNS sends the campaign summary.

## 1️⃣1️⃣ Customize the Email
...
After changing the template, run:

```bash
terraform apply
```

Terraform will package the updated template with the Email Processor Lambda.

## 1️⃣2️⃣ Monitor Lambda

```bash
aws logs tail   /aws/lambda/mailflow-email-system-processor   --since 10m   --region ap-south-1
```

## 1️⃣3️⃣ Destroy the Infrastructure
...
Because S3 versioning is enabled, uploaded objects and their versions may need to be removed before Terraform can delete the buckets.
